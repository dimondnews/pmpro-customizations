<?php 
	...
	if(empty($current_user->membership_level))
		...
		$confirmation_message = "<p>" . sprintf(__('Thank you! Your membership in %s is now active.', 'paid-memberships-pro' ), get_bloginfo("name"), $current_user->membership_level->name) . "</p>";		
...
<?php if(!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>		
	...
	<?php
		...
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your membership account and a receipt for your payment. A welcome email with a receipt has been sent to %s.', 'paid-memberships-pro' ), $pmpro_invoice->user->user_email) . "</p>";
	...
	<h3>
		<?php printf(__('Payment on %s', 'paid-memberships-pro' ), date_i18n(get_option('date_format'), $pmpro_invoice->timestamp));?>		
	</h3>
...
<?php do_action("pmpro_invoice_bullets_top", $pmpro_invoice); ?>
		...
		<?php if($current_user->membership_level->enddate) { ?>
			<h4><strong><?php _e('Membership Expires', 'paid-memberships-pro' );?>:</strong> <?php echo date_i18n(get_option('date_format'), $current_user->membership_level->enddate)?></h4>
		<?php } ?>


		<span><strong><?php _e('Account', 'paid-memberships-pro' );?>:</strong> <?php echo $current_user->display_name?> (<?php echo $current_user->user_email?>)</span><br />
		<span><strong><?php _e('Membership Level', 'paid-memberships-pro' );?>:</strong> <?php echo $current_user->membership_level->name?></span><br />


		<?php if($pmpro_invoice->getDiscountCode()) { ?>
			<span><strong><?php _e('Sponsor Code', 'paid-memberships-pro' );?>:</strong> <?php echo $pmpro_invoice->discount_code->code?></span>
		<?php } ?>
...
<?php 
	...
	{
		$confirmation_message .= "<p>" . sprintf(__('Below are details about your membership account and a receipt for your payment. A welcome email with a receipt has been sent to %s.', 'paid-memberships-pro' ), $current_user->user_email) . "</p>";
...
<span><strong><?php _e('Account', 'paid-memberships-pro' );?>:</strong> <?php echo $current_user->display_name?> (<?php echo $current_user->user_email?>)</span><br />
<span><strong><?php _e('Membership Level', 'paid-memberships-pro' );?>:</strong> <?php if(!empty($current_user->membership_level)) echo $current_user->membership_level->name; else _e("Pending", 'paid-memberships-pro' );?></span>
...
<nav id="nav-below" class="navigation" role="navigation">
	<div class="nav-next alignright">
		...
		<?php } else { ?>
			<?php _e('If your account is not activated within a few minutes, please contact us.', 'paid-memberships-pro' );?>
		<?php } ?>
	</div>
</nav>
